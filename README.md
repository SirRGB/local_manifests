...
AxxLineage.xml mostly refers to official Lineage device trees if available.
...
```
```
curl https://raw.githubusercontent.com/SirRGB/local_manifests/main/$OEM/$DEVICE/$MANIFEST_NAME > .repo/local_manifests/manifest.xml
```
be sure to replace $DEVICE and $MANIFEST_NAME with a device codename and manifest name, that are available.

Example:
```
mkdir .repo/local_manifests && curl https://raw.githubusercontent.com/SirRGB/local_manifests/main/motorola/griffin/A11Lineage.xml > .repo/local_manifests/manifest.xml
```
